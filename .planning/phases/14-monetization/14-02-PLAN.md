...
user_setup:
  - service: stripe
    ...
    dashboard_config:
      ...
      - task: "On Stripe products for Pro and Enterprise, add metadata key 'tier' with value 'pro' or 'enterprise'"
        location: "Stripe Dashboard -> Products -> Select product -> Metadata"
must_haves:
  truths:
    ...
    - "checkout.session.completed resolves tenant via eid from Stripe session metadata, then creates/updates org subscription with correct tier"
  ...
  key_links:
    ...
    - from: "apps/api/src/billing/webhook-handler.service.ts"
      to: "packages/database/src/repositories/drizzle-repositories.ts"
      via: "tenantRepo.findByEid to resolve eid from Stripe metadata to tenantId"
      pattern: "tenantRepo\\.findByEid"
...
Purpose: This is the bridge between Stripe's payment infrastructure and the app's tier system. When a customer pays via Stripe Checkout on the external pricing page, a webhook event flows through this module to update the org's subscription tier in real-time. The external pricing page creates the Stripe Checkout session (per user decision: "purchase happens outside the MCE iframe"), embedding the tenant's `eid` in session metadata so the webhook can resolve which org to provision.
...
<task type="auto">
  ...
  <action>
    ...
    **Step 3: Create apps/api/src/billing/webhook-handler.service.ts**
    ...
    - TENANT_REPOSITORY (ITenantRepository) -- needed to resolve eid from Stripe metadata to tenantId
    - AuditService
    - Logger
    ...
    2. Try/catch block routes event by type:
       - `checkout.session.completed`: Extract customer ID and subscription ID from session. Read `eid` from `session.metadata.eid` (the external pricing page embeds the tenant's Enterprise ID in Stripe Checkout session metadata -- per user decision, the purchase happens on the external website, which receives the eid via URL parameter from the app's upgrade CTA). Resolve eid to tenantId via `tenantRepo.findByEid(eid)`. If tenant not found, log error and throw (Stripe will retry). Update org_subscriptions by tenantId: stripeCustomerId, stripeSubscriptionId, tier based on price/product metadata. Log audit event `subscription.created`.
    ...
    **Step 5: Create apps/api/src/billing/billing.module.ts**
    ...
    - Providers: StripeProvider, BillingController, WebhookHandlerService, ORG_SUBSCRIPTION_REPOSITORY factory, STRIPE_WEBHOOK_EVENT_REPOSITORY factory, TENANT_REPOSITORY factory
  ...
  <done>
    BillingModule created with StripeProvider, BillingController (webhook endpoint), WebhookHandlerService (idempotent processing of 6 event types with eid-based tenant resolution). fastify-raw-body registered in main.ts. Webhook endpoint bypasses session/CSRF auth and uses Stripe signature verification. No dead code -- all services have consumers.
  </done>
</task>
...
<task type="auto">
  ...
  <action>
    **webhook-handler.service.unit.test.ts:**
    ...
    2. **checkout.session.completed**: Process a mock Stripe CheckoutSession event with `metadata.eid` set. Verify tenantRepo.findByEid called with correct eid. Verify orgSubscriptionRepo.updateFromWebhook called with resolved tenantId, correct tier, stripeCustomerId, stripeSubscriptionId. Verify webhookEventRepo.markCompleted called.
    3. **checkout.session.completed with unknown eid**: When tenantRepo.findByEid returns undefined, verify error is thrown (so Stripe retries) and markFailed is called.
    4. **customer.subscription.updated**: Process a mock subscription update event. Verify tier mapping works (active -> pro/enterprise based on metadata, past_due -> keeps current tier). Verify currentPeriodEnds updated.
    5. **customer.subscription.deleted**: Process a mock subscription deletion. Verify orgSubscriptionRepo.findByStripeCustomerId called with correct customer ID. Verify tier set to "free", stripeSubscriptionId cleared.
    6. **invoice.payment_failed**: Process a failed payment event. Verify NO tier change (graceful degradation). Verify audit event logged.
    7. **Error handling**: When handler throws, verify markFailed is called with event ID and error message, and error is re-thrown.
    8. **Unhandled event type**: Process an unknown event type. Verify markCompleted called (acknowledge but no action).
  ...
  <verify>
    ...
    - All test cases pass including idempotency, eid resolution, error handling, and event routing
  </verify>
  <done>
    WebhookHandlerService has unit tests for all 6 event types plus idempotency, eid resolution, and error handling. BillingController has unit tests for webhook endpoint including signature verification and edge cases.
  </done>
</task>
...
- checkout.session.completed resolves tenant via eid from Stripe session metadata
- customer.subscription.deleted uses findByStripeCustomerId to locate org
- Unit tests cover idempotency, eid resolution, error handling, and all event types
