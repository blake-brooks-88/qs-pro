...
must_haves:
  truths:
    ...
    - "Backend tests verify free-tier POST /query-activities/link/:savedQueryId returns 403 (no deployToAutomation)"
    - "Backend tests verify Pro-tier POST /query-activities/link/:savedQueryId succeeds (has deployToAutomation)"
  ...
  artifacts:
    ...
    - path: "apps/api/src/saved-queries/__tests__/saved-queries-stale.integration.test.ts"
      ...
      min_lines: 50
    - path: "apps/api/src/query-activities/__tests__/query-activities-deploy-gating.integration.test.ts"
      provides: "Integration tests confirming deployToAutomation gates link/publish/drift/blast (unchanged by Phase 18)"
      min_lines: 60
  ...
  key_links:
    ...
    - from: "apps/api/src/query-activities/__tests__/query-activities-deploy-gating.integration.test.ts"
      to: "apps/api/src/query-activities/query-activities.controller.ts"
      via: "Tests confirm deployToAutomation still gates link/publish/drift/blast endpoints"
      pattern: "deployToAutomation|FEATURE_NOT_ENABLED"
...
<objective>
Write comprehensive tests for all Phase 18 behavior: backend integration tests for shared folder CRUD, stale detection, and confirmation that deployToAutomation still gates link/publish/drift/blast (unchanged); frontend component tests for the two-section sidebar, stale warning dialog, and share confirmation dialog.

Purpose: Verify all locked decisions are correctly implemented and prevent regressions. Critically, confirm that link/publish/drift/blast gating is UNCHANGED from pre-Phase-18 behavior (deployToAutomation, NOT teamCollaboration).
...
<task type="auto">
  <name>Task 1: Backend integration tests for shared folders, stale detection, and deploy gating confirmation</name>
  <files>
    apps/api/src/folders/__tests__/folders-shared.integration.test.ts
    apps/api/src/saved-queries/__tests__/saved-queries-stale.integration.test.ts
    apps/api/src/query-activities/__tests__/query-activities-deploy-gating.integration.test.ts
...
**3. Create `query-activities-deploy-gating.integration.test.ts`:**

This file confirms that `deployToAutomation` continues to gate link/publish/drift/blast endpoints — Phase 18 did NOT change this gating. Per the locked CONTEXT.md decision: "deployToAutomation continues to gate linking as it does today — no re-gating migration needed."

Test cases:

**Free-tier (deployToAutomation=false) — all query-activity endpoints blocked:**
- `POST /query-activities/link/:savedQueryId` returns 403 FEATURE_NOT_ENABLED for free-tier tenant
- `DELETE /query-activities/link/:savedQueryId` (unlink) returns 403 FEATURE_NOT_ENABLED for free-tier tenant
- `POST /query-activities/publish/:savedQueryId` returns 403 FEATURE_NOT_ENABLED for free-tier tenant
- `GET /query-activities/drift/:savedQueryId` returns 403 FEATURE_NOT_ENABLED for free-tier tenant
- `GET /query-activities/blast-radius/:savedQueryId` returns 403 FEATURE_NOT_ENABLED for free-tier tenant
- `POST /query-activities` (deploy/create) returns 403 FEATURE_NOT_ENABLED for free-tier tenant

**Pro-tier (deployToAutomation=true, teamCollaboration=false) — all query-activity endpoints SUCCEED:**
- `POST /query-activities` (deploy/create) succeeds for Pro-tier tenant
- `GET /query-activities` succeeds for Pro-tier tenant
- `POST /query-activities/link/:savedQueryId` succeeds for Pro-tier tenant
- `DELETE /query-activities/link/:savedQueryId` (unlink) succeeds for Pro-tier tenant
- `POST /query-activities/publish/:savedQueryId` succeeds for Pro-tier tenant
- `GET /query-activities/drift/:savedQueryId` succeeds for Pro-tier tenant
- `GET /query-activities/blast-radius/:savedQueryId` succeeds for Pro-tier tenant

**Enterprise-tier (deployToAutomation=true, teamCollaboration=true) — all endpoints succeed:**
...
**Key assertion:** Pro-tier users CAN link/publish/drift/blast. The gating is `deployToAutomation` (Pro+Enterprise), NOT `teamCollaboration` (Enterprise-only). This confirms that Phase 18 did not introduce a regression.
...
Follow Arrange-Act-Assert pattern. Use behavioral assertions (test response body, not mock calls). Use existing RLS test helpers (`withRlsContext`, etc.).
  ...
  <verify>
Run `pnpm --filter api test` to verify all tests pass. Verify at least 20 test cases across all three files. Check that tests cover the locked decisions: feature gating, RLS visibility, stale detection, deployToAutomation gating preserved for link/publish/drift/blast.
  </verify>
  <done>
...
- `query-activities-deploy-gating.integration.test.ts` has 12+ test cases confirming: free-tier blocked (no deployToAutomation), Pro-tier succeeds (has deployToAutomation), Enterprise-tier succeeds
...
5. Tests cover all locked decisions: feature gating, RLS, stale detection, link hiding, enhanced delete, deployToAutomation gating preserved for link/publish/drift/blast
</verification>

<success_criteria>
- Comprehensive test coverage for all Phase 18 features
- Backend integration tests verify real database behavior (not mocks)
- Deploy-gating tests confirm Pro CAN link/publish/drift/blast (deployToAutomation), free tier cannot
