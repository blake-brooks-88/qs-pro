...
<objective>
Add database foundation for shared query workspaces: visibility column on folders, conditional RLS policy, teamCollaboration feature key, and shared-types updates.

Purpose: This is the foundational layer — all subsequent plans depend on the visibility column, the RLS policy, and the feature key existing.
Output: Migration file, updated Drizzle schema, updated shared-types for folders/saved-queries/features.
</objective>
...
3. `pnpm typecheck` passes across all packages (no downstream type breaks)
4. Migration file `0029_shared_query_workspaces.sql` exists with correct DDL
5. `teamCollaboration` appears only in Enterprise tier features
6. `SavedQueryResponseSchema` includes `latestVersionHash` field
</verification>
