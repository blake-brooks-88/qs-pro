...
files_modified:
  ...
  - packages/database/drizzle/<generated>_shared_query_workspaces.sql (via pnpm db:generate — NOT hand-written)
  - packages/database/drizzle/<generated>_shared_folders_rls_policy.sql (via drizzle-kit generate --custom — RLS only)
...
must_haves:
  ...
  artifacts:
    - path: "packages/database/drizzle/<generated>.sql"
      provides: "Drizzle Kit-generated migration for columns + index (NOT hand-written)"
      contains: "visibility"
    - path: "packages/database/drizzle/<generated>_shared_folders_rls_policy.sql"
      provides: "Custom migration for RLS policy only (--custom flag, the ONE exception)"
      contains: "folders_visibility_policy"
  ...
  key_links:
    - from: "packages/database/src/schema.ts"
      to: "packages/database/drizzle/<generated>.sql"
      via: "pnpm db:generate produces migration from schema diff — never hand-write DDL"
...
<task type="auto">
  ...
  <files>
    packages/database/src/schema.ts
    packages/database/drizzle/<generated>_shared_query_workspaces.sql (via pnpm db:generate)
    packages/database/drizzle/<generated>_shared_folders_rls_policy.sql (via drizzle-kit generate --custom)
  </files>
  <action>
...
**2. Generate migration via Drizzle Kit CLI:**

**CRITICAL: No manual/hand-written migrations are allowed.** All migrations MUST be generated using the Drizzle Kit CLI. Never create or edit SQL migration files by hand.

After updating `schema.ts` in step 1, generate the migration:
```bash
pnpm db:generate
```

This will auto-generate a migration file with the `ALTER TABLE` statements for the new columns and index based on the schema diff. Review the generated SQL to confirm it includes:
- `ALTER TABLE "folders" ADD COLUMN "visibility" ...`
- `ALTER TABLE "saved_queries" ADD COLUMN "updated_by_user_id" ...`
- `CREATE INDEX "folders_visibility_idx" ...`

**3. Generate a SEPARATE custom migration for RLS policy changes:**

Drizzle Kit cannot generate RLS policy DDL — this is the ONE case where `--custom` is required:
```bash
cd packages/database && npx drizzle-kit generate --custom --name=shared_folders_rls_policy
```

Then write ONLY the RLS SQL into the generated file:
```sql
...
**DO NOT put column/index DDL in the custom migration.** Only RLS policy statements go here — everything else is Drizzle Kit's job.
...
- The `saved_queries` RLS is already broadened to `tenant_id + mid` (migration 0023), so shared queries within shared folders are already visible BU-wide. No saved_queries RLS change needed.
  ...
  <verify>
Run `pnpm db:generate` — it should report no pending changes (schema and migrations are in sync). Run `pnpm --filter @qpp/database build` to verify the schema compiles. Confirm TWO migration files were generated: one auto-generated (columns + index) and one custom (RLS policy only).
  </verify>
  <done>
...
- Drizzle Kit-generated migration adds visibility column, updated_by_user_id column, and visibility index
- Separate custom migration (RLS only) replaces folders_user_isolation with conditional folders_visibility_policy
...
5. Drizzle Kit-generated migration exists for columns + index (via `pnpm db:generate`, NOT hand-written)
6. Custom migration exists for RLS policy only (via `drizzle-kit generate --custom`)
