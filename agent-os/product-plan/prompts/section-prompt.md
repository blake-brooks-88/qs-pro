...
- **SECTION_NAME** = [Human-readable name, e.g., "Editor Workspace"]
- **SECTION_ID** = [Folder name in sections/, e.g., "editor-workspace"]
...
0. **UI & Icon System**
   - Confirm icons must use `@solar-icons/react` only (do not add/use `lucide-react`)
